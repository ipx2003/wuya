    ...
    list.forEach(item => {
        ...
        portsToGenerate.forEach(({ port, tls }) => {
            if (tls) {
                ...
                wsParams.set('alpn', alpnParam || getALPN(echConfig));
                if (echConfig) {
...
function generateLinksFromNewIPs(list, user, workerDomain, customPath = '/', echConfig = null, alpnParam = null) {
    ...
    const wsPath = customPath || '/';
    const proto = 'vless';
    const alpnEncoded = encodeURIComponent(alpnParam || getALPN(echConfig));
    const alpnSuffix = `&alpn=${alpnEncoded}`;
    const echSuffix = echConfig ? `${alpnSuffix}&ech=${encodeURIComponent(echConfig)}` : alpnSuffix;
        ...
        <div class="card">
            ...
            <div class="form-group" style="margin-top: 12px;">
                ...
                <select id="customALPN" style="font-size: 14px; width: 100%; padding: 10px 12px; border-radius: 10px; border: 1px solid #d1d1d6; background: #f2f2f7; appearance: auto;">
                    <option value="h3,h2,http/1.1" selected>h3,h2,http/1.1（默认推荐）</option>
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (pathMatch) {
            ...
            const customALPNParam = url.searchParams.get('alpn') || 'h3,h2,http/1.1';
            const echConfig = echEnabled ? `${customECHDomainParam}+${customALPNParam}+${customDNSParam}` : null;
            const alpnParam = echEnabled ? null : customALPNParam; // 非ECH时单独传递ALPN
