...
// 从 echConfig 字符串中提取 ALPN
function getALPN(echConfig) {
    if (!echConfig) return '';
    // 格式: domain+alpn+dns 或 domain+dns（旧格式）
    const parts = echConfig.split('+');
    if (parts.length >= 3) return decodeURIComponent(parts[1]);
    if (parts.length === 2) {
        const second = parts[1];
        if (second.startsWith('http') || second.startsWith('udp')) return '';
...
// 生成VLESS链接
function generateLinksFromSource(list, user, workerDomain, disableNonTLS = false, customPath = '/', echConfig = null, alpnParam = null) {
    ...
    list.forEach(item => {
        ...
        portsToGenerate.forEach(({ port, tls }) => {
            const wsPath = customPath.replace(/AP_PORT/g, `/${item.ip}:${port}`);
            if (tls) {
                ...
                const effectiveALPN = alpnParam || getALPN(echConfig);
                if (effectiveALPN) wsParams.set('alpn', effectiveALPN);
                if (echConfig) {
...
// 生成Trojan链接
async function generateTrojanLinksFromSource(list, user, workerDomain, disableNonTLS = false, customPath = '/', echConfig = null, alpnParam = null) {
    const CF_HTTP_PORTS = [80, 8080, 8880, 2052, 2082, 2086, 2095];
    const CF_HTTPS_PORTS = [443, 2053, 2083, 2087, 2096, 8443];
    const defaultHttpsPorts = [443];
    const defaultHttpPorts = disableNonTLS ? [] : [80];
...
function generateLinksFromNewIPs(list, user, workerDomain, customPath = '/', echConfig = null, alpnParam = null) {
    const CF_HTTP_PORTS = [80, 8080, 8880, 2052, 2082, 2086, 2095];
    const CF_HTTPS_PORTS = [443, 2053, 2083, 2087, 2096, 8443];
    const links = [];
    const proto = 'vless';
    const alpnVal = alpnParam || getALPN(echConfig);
    const alpnSuffix = alpnVal ? `&alpn=${encodeURIComponent(alpnVal)}` : '';
    const echSuffix = (alpnVal || echConfig) ? (alpnSuffix + (echConfig ? `&ech=${encodeURIComponent(echConfig)}` : '')) : '';
...
// 生成订阅内容
async function handleSubscriptionRequest(request, user, customDomain, piu, ipv4Enabled, ipv6Enabled, ispMobile, ispUnicom, ispTelecom, evEnabled, etEnabled, vmEnabled, disableNonTLS, customPath, echConfig = null, alpnParam = null) {
    ...
    async function addNodesFromList(list) {
        ...
        if (useVL) {
            finalLinks.push(...generateLinksFromSource(list, user, nodeDomain, disableNonTLS, wsPath, echConfig, alpnParam));
        }
        if (etEnabled) {
            finalLinks.push(...await generateTrojanLinksFromSource(list, user, nodeDomain, disableNonTLS, wsPath, echConfig, alpnParam));
        }
        if (vmEnabled) {
            finalLinks.push(...generateVMessLinksFromSource(list, user, nodeDomain, disableNonTLS, wsPath, echConfig, alpnParam));
        }
    }
            ...
            if (piu && piu.toLowerCase().startsWith('https://')) {
                ...
                if (优选API的IP && 优选API的IP.length > 0) {
                    ...
                    if (IP列表.length > 0) {
                        ...
                        if (useVL) {
                            finalLinks.push(...generateLinksFromNewIPs(IP列表, user, nodeDomain, wsPath, echConfig, alpnParam));
                        }
                    }
                }
                ...
                if (newIPList.length > 0) {
                    ...
                    if (useVL) {
                        finalLinks.push(...generateLinksFromNewIPs(newIPList, user, nodeDomain, wsPath, echConfig, alpnParam));
                    }
                }
            }
        } catch (error) {
...
<body>
        ...
        <div class="card">
            ...
            <div class="form-group">
                ...
                <div style="display: flex; gap: 8px; align-items: center;">
                    <select id="pathPrefix" onchange="toggleCustom('pathPrefix', 'pathPrefixCustom')" style="font-size: 14px; flex: 1; padding: 10px 12px; border-radius: 10px; border: 1px solid #d1d1d6; background: #f2f2f7; appearance: auto;">
                        <option value="">无前缀</option>
                        <option value="UUID_PATH">[/UUID] UUID模式</option>
                        <option value="AP_PORT">[AP:端口] 仅IP端口</option>
                        <option value="custom">自定义前缀</option>
                    </select>
                    <span style="font-size: 14px; color: #86868b;">+</span>
                    <select id="customPath" onchange="toggleCustom('customPath', 'pathSuffixCustom')" style="font-size: 14px; flex: 1; padding: 10px 12px; border-radius: 10px; border: 1px solid #d1d1d6; background: #f2f2f7; appearance: auto;">
                        <option value="">（空）</option>
                        <option value="/">/（无参数）</option>
                        <option value="/?ed=2560">/?ed=2560</option>
                        <option value="/?ed=2048">/?ed=2048</option>
                        <option value="custom">自定义后缀</option>
                    </select>
                </div>
                <input type="text" id="pathPrefixCustom" placeholder="自定义前缀，如 /abc" style="display:none; margin-top:8px; font-size:14px;">
                <input type="text" id="pathSuffixCustom" placeholder="自定义后缀，如 /v2ray 或 /?ed=2560" style="display:none; margin-top:8px; font-size:14px;">
            </div>
            ...
            <div class="form-group" style="margin-top: 12px;">
                <label>ALPN</label>
                <select id="customALPN" style="font-size: 14px; width: 100%; padding: 10px 12px; border-radius: 10px; border: 1px solid #d1d1d6; background: #f2f2f7; appearance: auto;">
                    <option value="">未启用</option>
                    <option value="h3,h2,http/1.1">h3,h2,http/1.1（默认推荐）</option>
            ...
            <div class="form-group" id="echOptionsGroup" style="margin-top: 12px; display: none;">
                <label>EchConfig DNS 服务 (DoH/UDP)</label>
                <select id="customDNS" onchange="toggleCustom('customDNS', 'echDNSCustom')" style="font-size: 14px; width: 100%; padding: 10px 12px; border-radius: 10px; border: 1px solid #d1d1d6; background: #f2f2f7; appearance: auto;">
                    <option value="">未启用</option>
                    <option value="https://alidns.com">https://alidns.com（阿里1 DoH：国内推荐，小白首选）</option>
                    <option value="https://dns.alidns.com/dns-query">https://dns.alidns.com/dns-query（阿里2 DoH：国内推荐，小白首选）</option>
                    <option value="https://dns.joeyblog.eu.org/joeyblog">https://dns.joeyblog.eu.org/joeyblog（joeyblog DoH：国内推荐，小白首选）</option>
                    <option value="https://cmliussss.net">https://cmliussss.net（Cloudflare DoH：CM镜像）</option>
                    <option value="https://cmliussss.com">https://cmliussss.com（Google DoH：CM镜像）</option>
                    <option value="udp://208.67.220.220:443">udp://208.67.220.220:443（OpenDNS）</option>
                    <option value="udp://149.112.112.112:9953">udp://149.112.112.112:9953（Quad9）</option>
                    <option value="udp://45.90.28.0:5353">udp://45.90.28.0:5353（NextDNS）</option>
                    <option value="udp://188.166.206.224:5003">udp://188.166.206.224:5003（Tiarap）</option>
                    <option value="https://applied-privacy.net">https://applied-privacy.net（Applied Privacy DoH）</option>
                    <option value="https://doh.pub">https://doh.pub（腾讯国密 DoH）</option>
                    <option value="https://360.cn">https://360.cn（360 DoH）</option>
                    <option value="https://onedns.net">https://onedns.net（OneDNS DoH）</option>
                    <option value="custom">—— 自定义输入 ——</option>
                </select>
                <input type="text" id="echDNSCustom" placeholder="请输入 udp:// 或 https:// 地址" style="display:none; margin-top:8px; font-size:14px;">
                
                <label style="margin-top: 12px; display: block;">EchConfig 解析域名 (ECH SNI)</label>
                <select id="customECHDomain" onchange="toggleCustom('customECHDomain', 'echSNICustom')" style="font-size: 14px; width: 100%; padding: 10px 12px; border-radius: 10px; border: 1px solid #d1d1d6; background: #f2f2f7; appearance: auto;">
                    <option value="">未启用</option>
                    <option value="cloudflare-ech.com">cloudflare-ech.com（Cloudflare ECH域名：小白首选）</option>
                    <option value="crypto.cloudflare.com">crypto.cloudflare.com（Cloudflare密码学服务）</option>
                    <option value="encryptedsni.com">encryptedsni.com（Cloudflare加密SNI）</option>
                    <option value="icook.hk">icook.hk（愛料理 HK）</option>
                    <option value="cm.edu.kg">cm.edu.kg（CM科技大学）</option>
                    <option value="godotengine.org">godotengine.org（Godot 开源游戏引擎）</option>
                    <option value="www.britannica.com">www.britannica.com（大英百科全书）</option>
                    <option value="www.prometheus.io">www.prometheus.io（Prometheus 监控系统）</option>
                    <option value="www.kyocera.com">www.kyocera.com（Kyocera 集团官网）</option>
                    <option value="celestia.org">celestia.org（Celestia 模块化区块链）</option>
                    <option value="lido.fi">lido.fi（Lido 流动质押）</option>
                    <option value="custom">—— 自定义输入 ——</option>
                </select>
                <input type="text" id="echSNICustom" placeholder="请输入 ECH 解析域名" style="display:none; margin-top:8px; font-size:14px;">
            </div>
        </div>
    ...
    <script>
        ...
        function getEffectivePath() {
            const prefixSelect = document.getElementById('pathPrefix');
            const suffixSelect = document.getElementById('customPath');
            if (!prefixSelect || !suffixSelect) return '/';
            
            let prefix = (prefixSelect.value === 'custom')
                ? (document.getElementById('pathPrefixCustom')?.value?.trim() || '')
                : prefixSelect.value;
            let suffix = (suffixSelect.value === 'custom')
                ? (document.getElementById('pathSuffixCustom')?.value?.trim() || '/')
                : suffixSelect.value;
            
            const uuid = document.getElementById('uuid')?.value?.trim() || '';
            
            // AP_PORT 和 UUID_PATH 透传到服务端处理
            // UUID_PATH 客户端也可提前解析
            if (prefix === 'UUID_PATH') prefix = uuid ? '/' + uuid : '';
            // AP_PORT 保持占位符，服务端会用实际 IP:端口 替换
        ...
        function generateClientLink(clientType, clientName) {
            ...
            const uuid = document.getElementById('uuid').value.trim();
            const customPath = getEffectivePath();
            ...
            // ALPN 始终传输
            const alpnVal = document.getElementById('customALPN') && document.getElementById('customALPN').value.trim();
            if (alpnVal) subscriptionUrl += \`&alpn=\${encodeURIComponent(alpnVal)}\`;
            
            if (switches.switchECH) {
                subscriptionUrl += '&ech=yes';
                const dnsVal = getSelectOrCustom('customDNS', 'echDNSCustom');
                if (dnsVal) subscriptionUrl += \`&customDNS=\${encodeURIComponent(dnsVal)}\`;
                const domainVal = getSelectOrCustom('customECHDomain', 'echSNICustom');
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (pathMatch) {
            ...
            const customDNSParam = url.searchParams.get('customDNS') || (echEnabled ? customDNS : '');
            const customECHDomainParam = url.searchParams.get('customECHDomain') || (echEnabled ? customECHDomain : '');
            const customALPNParam = url.searchParams.get('alpn') || '';
            const echConfig = echEnabled ? `${customECHDomainParam}+${customALPNParam}+${customDNSParam}` : null;
            const alpnParam = echEnabled ? null : (customALPNParam || null); // 非ECH时，ALPN为空就不传

            // 自定义路径
            const customPath = url.searchParams.get('path') || '/';

            return await handleSubscriptionRequest(request, uuid, domain, piu, ipv4Enabled, ipv6Enabled, ispMobile, ispUnicom, ispTelecom, evEnabled, etEnabled, vmEnabled, disableNonTLS, customPath, echConfig, alpnParam);
        }
